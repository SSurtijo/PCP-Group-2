...
def get_maturity_label(rating):
    """Convert numeric CMM rating to maturity label with color."""
    # Adjusted for 1-4 scale range
...
def render_l2_domains_table(selected_company_id=None):
    """Render the L2 Domains table showing NIST CSF maturity by domain."""
    st.subheader("NIST CSF L2 Domains - Maturity Assessment")
    ...
    try:
        ...
        if selected_company_id is not None and "company_id" in df_cmm.columns:
            ...
            try:
                df_cmm = df_cmm[df_cmm["company_id"].astype(str) == str(selected_company_id)]
        ...
        for api_domain_name in unique_domains:
            # Get all controls for this domain from API
            domain_data = df_cmm[df_cmm["domain"] == api_domain_name]

            # Extract ratings (excluding null values)
            ratings_for_domain = []
            ...
            # Calculate average rating for this domain if we have data
            if ratings_for_domain:
                avg_rating = sum(ratings_for_domain) / len(ratings_for_domain)

                # Get function from first control_ref in this domain
                try:
                ...
                domain_ratings.append({
                    ...
                    "domain": api_domain_name,  # Use actual API domain name
        ...
        try:
            # Ensure function column is string type before categorical conversion
            domain_ratings["function"] = domain_ratings["function"].astype(str)

            # Create categorical with only functions that exist in our data
            existing_functions = [f for f in function_order if f in domain_ratings["function"].values]
            other_functions = [f for f in domain_ratings["function"].unique() if f not in function_order]
            all_categories = existing_functions + other_functions

            domain_ratings["function"] = pd.Categorical(
                domain_ratings["function"],
                categories=all_categories,
                ordered=True
            )
            domain_ratings = domain_ratings.sort_values(["function", "domain"])
        except (TypeError, ValueError) as e:
            # Fallback to simple sorting if categorical conversion fails
        ...
        try:
            # Ensure cmm_rating is numeric for filtering
            domain_ratings["cmm_rating"] = pd.to_numeric(domain_ratings["cmm_rating"], errors="coerce")
            ...
            with col2:
                marginal_count = len(domain_ratings[(domain_ratings["cmm_rating"] >= 2.0) & (domain_ratings["cmm_rating"] < 3.0)])
                st.metric("Marginal Domains", int(marginal_count))

            with col3:
                marginal_strong_count = len(domain_ratings[(domain_ratings["cmm_rating"] >= 3.0) & (domain_ratings["cmm_rating"] < 3.5)])
        ...
        display_df = domain_ratings[["function", "domain", "cmm_rating", "label", "indicator"]].copy()

        # Convert function column to string to avoid categorical issues
        ...
        for idx, row in merged_display_df.iterrows():
            if row["function"] == prev_function:
                merged_display_df.at[idx, "function"] = ""  # Empty string for subsequent occurrences
            else:
                prev_function = row["function"]

        # Rename columns
        merged_display_df = merged_display_df.rename(columns={
        ...
        # Display the table
        try:
            # Ensure Rating column is properly numeric for column config
            merged_display_df["Rating"] = pd.to_numeric(merged_display_df["Rating"], errors="coerce")
            ...
            st.warning(f"Error displaying table with formatting: {e}. Showing simple table.")
            st.dataframe(merged_display_df, width="stretch", hide_index=True)


    except Exception as e:
        st.error(f"Error loading L2 domains data: {str(e)}")
